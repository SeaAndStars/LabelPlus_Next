...
public partial class TranslateViewModel : ViewModelBase
{
    ...
    public void InitializeServices(IFileDialogService dialogs) { _dialogs ??= dialogs; Logger.Debug("Services initialized."); }
    ...
    private void StartAutoSave() { _autoSaveTimer ??= new Timer(async _ => await AutoSaveTickAsync(), null, _autoSaveInterval, _autoSaveInterval); Logger.Info("Auto-save started with interval {minutes} minutes.", _autoSaveInterval.TotalMinutes); }
    public void StopAutoSave() { _autoSaveTimer?.Dispose(); _autoSaveTimer = null; Logger.Info("Auto-save stopped."); }
    ...
    private async Task AutoSaveTickAsync()
    {
        ...
        try
        {
            ...
            string snapshot;
            try { snapshot = await BuildSnapshotAsync(); } catch (Exception ex) { Logger.Error(ex, "Autosave snapshot build failed."); return; }
            var sig = ComputeHash(snapshot);
            if (string.Equals(sig, _lastBackupSignature, StringComparison.Ordinal)) { Logger.Debug("Skip autosave: snapshot unchanged."); return; }
    ...
    public IReadOnlyCollection<string> GetIncludedImageFiles() { return LabelFileManager1.StoreManager.Store.Keys.ToList(); }
    public async Task AddImageFileAsync(string relativePath) { await LabelFileManager1.StoreManager.AddFileAsync(relativePath); Logger.Info("Image added to include list: {file}", relativePath); }
    public async Task RemoveImageFileAsync(string relativePath) { await LabelFileManager1.StoreManager.RemoveFileAsync(relativePath); Logger.Info("Image removed from include list: {file}", relativePath); }
    ...
    public (List<string> groupList, string notes) GetFileSettings() { return (new List<string>(LabelFileManager1.GroupStringList), LabelFileManager1.Comment); }
    public async Task<bool> SaveFileSettingsAsync(List<string> groupList, string notes) { LabelFileManager1.UpdateHeader(groupList, notes); if (!string.IsNullOrEmpty(OpenTranslationFilePath)) { await FileSave(OpenTranslationFilePath); return true; } return false; }
    public async Task AddLabelAtAsync(float xPercent, float yPercent, int category = 1)
    {
        ...
        UpdateCurrentLabels();
        if (CurrentLabels.Count > 0) { SelectedLabel = CurrentLabels[^1]; CurrentText = SelectedLabel.Text; }
    }
    ...
    {
        if (_dialogs is null) return;
        if (string.IsNullOrEmpty(OpenTranslationFilePath)) { await _dialogs.ShowMessageAsync("未打开翻译文件，无法保存。请使用‘另存为’。"); return; }
        try
        {
    ...
    partial void OnOpenTranslationFilePathChanged(string? value) { if (!string.IsNullOrEmpty(value)) StartAutoSave(); else StopAutoSave(); }
    partial void OnSelectedImageFileChanged(string? value)
    {
        ...
        if (!string.IsNullOrEmpty(value))
        {
            ...
            {
                try { PicImageSource = new Bitmap(imagePath); } catch { PicImageSource = null; }
            }
            else { PicImageSource = null; }
        }
        else { PicImageSource = null; }
        if (CurrentLabels.Count > 0) { SelectedLabel = CurrentLabels[0]; CurrentText = SelectedLabel.Text; }
        else { SelectedLabel = null; CurrentText = string.Empty; }
    }
    partial void OnSelectedLabelChanged(LabelItem? value) => CurrentText = value?.Text ?? string.Empty;
    partial void OnCurrentTextChanged(string? value) { if (SelectedLabel != null) { SelectedLabel.Text = value; LabelFileManager1.StoreManager.TouchDirty(); Logger.Debug("Label text updated: {text}", value); } }
    ...
    [RelayCommand] public async Task ManageImagesCommand()
    {
        ...
        if (string.IsNullOrEmpty(folder)) return;
        var selected = await _dialogs.ChooseImagesAsync(folder);
        if (selected == null) { await _dialogs.ShowMessageAsync("已取消。"); return; }
