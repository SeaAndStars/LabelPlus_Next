...
public class LabeledTextBox : TextBox
{
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (Index > 0)
        {
            ...
            var fontSize = 16d;
            var layout = new TextLayout(text, Typeface.Default, fontSize, IndexForegroundBrush, TextAlignment.Left, TextWrapping.NoWrap);
            double w = 0, h = 0;
            ...
            layout.Draw(context, new Avalonia.Point(cap.X + padX, cap.Y + padY));
        }

        if (capsule is Rect rc)
        {
            if (rc.X > 0)
                context.DrawLine(pen, new Avalonia.Point(0, lineY), new Avalonia.Point(rc.X, lineY));
            var rightStart = rc.Right;
            if (rightStart < bounds.Width)
                context.DrawLine(pen, new Avalonia.Point(rightStart, lineY), new Avalonia.Point(bounds.Width, lineY));
        }
        else
        {
            context.DrawLine(pen, new Avalonia.Point(0, lineY), new Avalonia.Point(bounds.Width, lineY));
        }
    }
}
